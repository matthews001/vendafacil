...
(() => {
  ...
  function v3NewDeliveryZone() {
    ...
    ['commerce-delivery-zone-id','commerce-delivery-zone-name','commerce-delivery-zone-fee','commerce-delivery-zone-minimum','commerce-delivery-zone-minutes'].forEach(id => $(id).value='');
  ...
  function v3EditDeliveryZone(id) {
    ...
    $('commerce-delivery-zone-title').textContent='Editar região de entrega'; $('commerce-delivery-zone-id').value=zone.id; $('commerce-delivery-zone-name').value=zone.name||''; $('commerce-delivery-zone-fee').value=Number(zone.fee||0).toFixed(2); $('commerce-delivery-zone-minimum').value=Number(zone.minimum_order||0)?Number(zone.minimum_order).toFixed(2):''; $('commerce-delivery-zone-minutes').value=zone.estimated_minutes||''; $('commerce-delivery-zone-active').checked=!!zone.active; openModal('modal-commerce-delivery-zone');
  }
  async function v3SaveDeliveryZone() {
    const id=$('commerce-delivery-zone-id').value, name=$('commerce-delivery-zone-name').value.trim(); const fee=Number(String($('commerce-delivery-zone-fee').value).replace(',','.')); const min=Number(String($('commerce-delivery-zone-minimum').value||'0').replace(',','.')); const raw=$('commerce-delivery-zone-minutes').value; const minutes=raw?Number(raw):null;
    ...
    const payload={business_id:state.business.id,name,fee,minimum_order:min||0,estimated_minutes:minutes,active:$('commerce-delivery-zone-active').checked,updated_at:new Date().toISOString()};
  ...
  function v3SelectedZone(){return v3StoreZones().find(z=>z.id===($('store-delivery-zone')?.value||''))||null;}
