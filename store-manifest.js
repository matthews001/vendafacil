...
module.exports = async (req, res) => {
  const name = cleanText(req.query?.nome, cleanSlug(req.query?.loja) || 'VendaFácil');
  const color = cleanColor(req.query?.cor);
  const initial = Array.from(name)[0]?.toUpperCase() || 'V';
