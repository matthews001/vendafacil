...
for(const token of ["CACHE_NAME = 'vendafacil-pwa-v15-cep-only'",'storefront.v7-cep-only.js','const responseForCache = response.clone();','event.waitUntil(caches.open(CACHE_NAME)']) if(!sw.includes(token)) throw new Error('Service Worker incompleto: '+token);
...
if(!vercel.includes('"source": "/sw.js"')||!vercel.includes('no-cache, no-store, must-revalidate')) throw new Error('Vercel precisa impedir cache do sw.js.');
console.log('PWA validado: cache seguro, assets v7 e sem alerta de beforeinstallprompt bloqueado.');
