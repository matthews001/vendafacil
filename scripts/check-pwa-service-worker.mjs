...
for(const token of ["CACHE_NAME = 'vendafacil-pwa-v23-order-hours'",'storefront.v14-stable.js','styles/theme-contrast.css','styles/contrast-audit.css','styles/mobile-responsive.css','const responseForCache = response.clone();','event.waitUntil(caches.open(CACHE_NAME)']) if(!sw.includes(token)) throw new Error('Service Worker incompleto: '+token);
...
console.log('PWA validado: cache seguro, tema responsivo versionado e sem alerta de beforeinstallprompt bloqueado.');
