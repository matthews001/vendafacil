...
for(const token of ['storefront.v10-payment-methods.js','vfStoreCepFallback','Frete calculado pelo CEP']) if(!store.includes(token)) throw new Error('Carregamento/checkout CEP ausente: '+token);
...
for(const token of ['storefront.v10-payment-methods.js','storefront.v10-payment-methods.css']) if(!build.includes(token)) throw new Error('Build não publica os assets CEP: '+token);
