...
for(const token of ['storefront.v12-stable.js','vfStoreCepFallback','Frete calculado pelo CEP']) if(!store.includes(token)) throw new Error('Carregamento/checkout CEP ausente: '+token);
...
for(const token of ['storefront.v12-stable.js','storefront.v12-stable.css']) if(!build.includes(token)) throw new Error('Build não publica os assets CEP: '+token);
