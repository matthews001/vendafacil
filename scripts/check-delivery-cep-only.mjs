...
for(const token of ['storefront.v11-core-flow.js','vfStoreCepFallback','Frete calculado pelo CEP']) if(!store.includes(token)) throw new Error('Carregamento/checkout CEP ausente: '+token);
...
for(const token of ['storefront.v11-core-flow.js','storefront.v11-core-flow.css']) if(!build.includes(token)) throw new Error('Build não publica os assets CEP: '+token);
