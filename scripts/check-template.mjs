...
for(const token of ['storefront.v11-core-flow.js','vfStoreCepFallback','store-delivery-cep','store-delivery-city','store-delivery-state','Frete calculado pelo CEP']) if(!store.includes(token)) throw new Error('Vitrine CEP incompleta: '+token);
...
for(const token of ['storefront.v11-core-flow.js','storefront.v11-core-flow.css']) if(!build.includes(token)) throw new Error('Build não publica os assets atuais da vitrine: '+token);
