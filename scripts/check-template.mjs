...
for(const token of ['storefront.v10-payment-methods.js','vfStoreCepFallback','store-delivery-cep','store-delivery-city','store-delivery-state','Frete calculado pelo CEP']) if(!store.includes(token)) throw new Error('Vitrine CEP incompleta: '+token);
...
for(const token of ['storefront.v10-payment-methods.js','storefront.v10-payment-methods.css']) if(!build.includes(token)) throw new Error('Build não publica os assets atuais da vitrine: '+token);
