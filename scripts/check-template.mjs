...
for(const token of ['screen-commerce-app','commerce-page-orders','commerce-page-pos','commerce-page-products','commerce-page-delivery','commerce-page-employees','vf-pdv-step7-script']) if(!html.includes(token)) throw new Error('Painel incompleto: '+token);
...
for(const token of ['storefront.v14-stable.js','vfStoreCepFallback','store-delivery-cep','store-delivery-city','store-delivery-state','Frete calculado pelo CEP']) if(!store.includes(token)) throw new Error('Vitrine CEP incompleta: '+token);
...
for(const token of ['storefront.v14-stable.js','storefront.v14-stable.css']) if(!build.includes(token)) throw new Error('Build não publica os assets atuais da vitrine: '+token);
